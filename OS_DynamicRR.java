...
public class OS_DynamicRR {
    ...
    public static int quantum; //quantum of exeution in CPU
    public static int i; //for external events
    public static int e; //for internal events
    public static int SR; //  Weighted sum of the remaining burst times in ready queue
    public static int AR; // Weighted average of the burst times
            ...
            switch (command[0]) {
                ...
                case "D":
                    ...
                    if (time != 999999) {
                        ...
                        quantum = first_job.getbusrtTime();
    ...
    public static void internalEvent() {
        ...
        if (executingJob.getNeedTime() <= 0) {
            ...
            if (!shortestQueue.isEmpty()) {
                ...
                //determine the quantum value
                quantum = Math.min(executingJob.getNeedTime(), AR);
            ...
            //else, the job still have burst time, start dynamic round robin 
            dynanicRounRobin();
        }
    }

    //-----------------------------UPDATE SR AND AR-----------------------------
    public static void updated_SR_AR() {
        // update the value of AR and SR
        if (!shortestQueue.isEmpty()) {
            AR = 0;
            SR = 0;
            // sum all the (needTime * weight) of each job
            for (JobNode p : shortestQueue) {
                SR += (p.getNeedTime() * p.getWeight());
            }
            //calculate the avetage by dividing on the number of jobs in the shortest queue
            AR = (SR / shortestQueue.size());
        }
    }
    ...
    public static void dynanicRounRobin() {
        if (shortestQueue.isEmpty()) {
            ...
            // update SR and AR
            updated_SR_AR();
            //take the first job from the shortest queue
            JobNode p = shortestQueue.poll();
            // set quantum time
            quantum = Math.min(p.getNeedTime(), AR);
            // set the job as the excuting job to be run next in the CPU
            executingJob = p;
            // set the job start time of execution
            executingJob.setStartTime(currentTime);
            // set the executing job finish time
...
//---------------------------------SORTING--------------------------------------
class sortbyID implements Comparator<JobNode> {
